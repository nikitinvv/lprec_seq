...
class Padj:
	def __init__(self,fZgpu,fZ,lp2p1,lp2p2,lp2p1w,lp2p2w,C2lp1,C2lp2,cids,lpids,wids, filter):
		self.fZgpu=fZgpu;
		self.fZ=fZ;
		self.lp2p1=lp2p1; self.lp2p2=lp2p2;
		self.lp2p1w=lp2p1w; self.lp2p2w=lp2p2w;
		self.C2lp1=C2lp1; self.C2lp2=C2lp2;
		self.cids=cids;
		self.lpids=lpids;
		self.wids=wids;
		self.filter=filter;
...
def create_adj(P,filter_type):
	...
	[x1,x2]=meshgrid(linspace(-1,1,P.N),linspace(-1,1,P.N)); x1=ndarray.flatten(x1); x2=ndarray.flatten(x2);
	cids=where(x1**2+x2**2<=1);
	C2lp1=[None]*P.Nspan; C2lp2=[None]*P.Nspan;
	for k in range(0,P.Nspan):
		...
		C2lp1[k]=arctan2(z2,z1); C2lp2[k]=log(sqrt(z1**2+z2**2));  

	#(lp2p1,lp2p2), transformed log-polar to polar coordinates
	[z1,z2]=meshgrid(P.thsp,exp(P.rhosp)); z1=ndarray.flatten(z1); z2=ndarray.flatten(z2);
	z2n=z2-(1-P.aR)*cos(z1);z2n=z2n/P.aR;
	lpids=where((z1>=-P.beta/2) & (z1<P.beta/2) &(abs(z2n)<=1));
	lp2p1=[None]*P.Nspan; lp2p2=[None]*P.Nspan;
	for k in range(0,P.Nspan):
	...
	z2n=exp(log(z2[wids])+log(P.am)-P.g)-(1-P.aR)*cos(z1[wids]);z2n=z2n/P.aR;
	...
	z2n2=exp(log(z2[wids2])-log(P.am)+P.g)-(1-P.aR)*cos(z1[wids2]); z2n2=z2n2/P.aR;
	lpidsw2=where((z1[wids2]>=-P.beta/2) & (z1[wids2]<P.beta/2) & (abs(z2n2)<=1+P.add*(P.rhosp[1]-P.rhosp[0])));
	lp2p1w=[None]*P.Nspan; lp2p2w=[None]*P.Nspan;
	for k in range(0,P.Nspan):
	...
	proj0=[None]*P.Nspan; projl=[None]*P.Nspan;
	for k in range(0,P.Nspan):
	...
	for k in range(0,P.Nspan):
		...
		lp2p2[k]=(lp2p2[k]+1)/2*(P.Ns-1);
		lp2p2w[k]=(lp2p2w[k]+1)/2*(P.Ns-1);
		C2lp1[k]=(C2lp1[k]-P.thsp[0])/(P.thsp[-1]-P.thsp[0])*(P.Ntheta-1);
		C2lp2[k]=(C2lp2[k]-P.rhosp[0])/(P.rhosp[-1]-P.rhosp[0])*(P.Nrho-1);
	
	const=(P.N+1)/float32(P.N)*(P.Ns-1)
	fZ=fZ[:,arange(0,P.Ntheta/2+1)]*const
	fZgpu=fZ/(P.B3com[:,arange(0,P.Ntheta/2+1)])
	#filter
	if (filter_type!='None'):
		filter=take_filter(P.Ns,filter_type);
	else: filter=None;

	Padj0=Padj(fZgpu,fZ,lp2p1,lp2p2,lp2p1w,lp2p2w,C2lp1,C2lp2,cids,lpids,wids,filter);
	return Padj0;
