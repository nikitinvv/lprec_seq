...
def create_gl(N,Nproj,Ns,Nslices):
	"initialize global parameters for the log-polar-based method and save to file"
	Nspan=3;
	proj=linspace(0,pi,Nproj+1);proj=proj[0:-1];
	s=linspace(-1,1,Ns);add=0;
	beta=pi/Nspan;
	#log-polar space
	(Nrho,Ntheta,dtheta,drho,aR,am,g)=getparameters(beta,proj[1]-proj[0],s[1]-s[0],N,size(proj),size(s));
	thsp=arange(-Ntheta/2,Ntheta/2)*dtheta;
	rhosp=arange(-Nrho,0)*drho;
	proj=proj-beta/2;

	#compensation for cubic interpolation
	B3th=splineB3(thsp,1);B3th=fft.fft(fft.ifftshift(B3th));
	B3rho=splineB3(rhosp,1);B3rho=(fft.fft(fft.ifftshift(B3rho)));
	B3com=array(transpose(matrix(B3rho))*B3th);

	Pgl0=Pgl(Nspan,N,proj,s,thsp,rhosp,aR,beta,add,B3com,am,g,Nslices);
	return Pgl0;

def getparameters(beta,dtheta,ds,N,Nproj,Ns):
